...
- **MCP 驅動的可擴充性：** 充分利用 coding agent 預設隨附 GitHub MCP server 這件事，並透過支援自訂 MCP servers 來擴充能力。
...
3. 產生你的 token
4. 透過環境變數 `GH_TOKEN` 或 `GITHUB_TOKEN`（依優先順序）把 token 加入你的環境
...
我們很高興你在 Copilot CLI 發展歷程的早期階段就加入我們。
