...
GitHub Copilot CLI 會把 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話建置、除錯並理解程式碼。它由與 GitHub 的 Copilot coding agent 相同的代理式框架驅動，在深度整合你的 GitHub 工作流程的同時提供智慧協助。
...
- **MCP 驅動的可延伸性：** 利用 coding agent 預設隨附 GitHub 的 MCP 伺服器，並支援自訂 MCP 伺服器以擴充能力。
- **完全掌控：** 每個動作都可在執行前預覽——沒有你的明確同意就不會發生。

我們仍在旅程初期，但有了你的回饋，我們正快速迭代，讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，可強化程式碼智慧功能。此功能提供跳轉到定義、懸停資訊與診斷等智慧功能。
