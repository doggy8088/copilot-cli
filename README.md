...
- **由 MCP 驅動的可擴充性：** 利用 coding agent 預設隨附 GitHub 的 MCP server 的能力，並透過自訂 MCP servers 擴充功能。
...
GitHub Copilot CLI 支援 Language Server Protocol（LSP），可提供更強的程式碼智慧功能。這項功能包含像是跳至定義、懸停資訊與診斷等智慧程式碼能力。
...
你的洞見非常寶貴！請在這個儲存庫中開啟 issue、參與 Discussions，並在 CLI 中執行 `/feedback` 來提交一份保密的意見調查！
