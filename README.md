...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你可以透過自然語言對話來建置、除錯與理解程式碼。它由與 GitHub Copilot coding agent 相同的 agent 型 harness 驅動，在深度整合你的 GitHub 工作流程的同時，提供智慧型協助。
...
我們正將 GitHub Copilot coding agent 的能力直接帶到你的終端機。透過 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI agent 協作。
...
- **由 MCP 驅動的可擴充性：** 充分利用 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers 來擴充能力。
- **完整掌控：** 在執行前先預覽每個動作，沒有你的明確核准就不會發生任何事。

我們仍在這段旅程的早期，但有了你的回饋，我們正快速迭代，讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
使用 `| sudo bash` 來以 root 身分執行，並安裝到 `/usr/local/bin`。

將 `PREFIX` 設為安裝到 `$PREFIX/bin/` 目錄。當以 root 執行時，預設為 `/usr/local`；當以非 root 使用者執行時，預設為 `$HOME/.local`。
...
啟用後，這個設定會持久化到你的 config 中，因此後續啟動時就不再需要 `--experimental` 旗標。

#### 實驗功能

- **Autopilot mode：** Autopilot 是一種新模式（按 `Shift+Tab` 可循環切換模式），會鼓勵 agent 持續工作，直到任務完成。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，以增強程式碼智慧功能。這項功能提供例如前往定義、懸停資訊與診斷等智慧程式碼能力。
...
你的見解非常寶貴！請在此 repo 中開 issue、加入 Discussions，並從 CLI 執行 `/feedback` 來提交一份保密的回饋問卷！
