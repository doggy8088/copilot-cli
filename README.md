...
GitHub Copilot CLI 把 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯與理解程式碼。它採用與 GitHub 的 Copilot coding agent 相同的代理式架構，在深度整合你的 GitHub 工作流程的同時提供智慧協助。
...
我們把 GitHub Copilot coding agent 的力量直接帶到你的終端機。使用 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI 代理協作。
...
設定 `PREFIX` 以安裝到 `$PREFIX/bin/` 目錄。預設值為 root 使用者的 `/usr/local`，或非 root 使用者的 `$HOME/.local`。
