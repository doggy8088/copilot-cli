...
GitHub Copilot CLI 將 AI 驅動的程式設計輔助直接帶到你的命令列，讓你能透過自然語言對話建置、除錯並理解程式碼。它採用與 GitHub Copilot coding agent 相同的代理式執行框架，在與 GitHub 工作流程深度整合的同時，提供智慧協助。
...
我們正將 GitHub Copilot coding agent 的強大能力直接帶進你的終端機。有了 GitHub Copilot CLI，你可以在本機與一個理解你的程式碼與 GitHub 情境的 AI 代理同步協作。
...
- **完整掌控：** 每個動作在執行前都可預覽，未經你的明確核准，不會進行任何操作。

我們仍處於發展早期，但有了你的回饋，我們正快速迭代，讓 GitHub Copilot CLI 成為你終端機中最好的夥伴。
...
預設情況下，`copilot` 使用 Claude Sonnet 4.5。執行 `/model` slash command，從其他可用模型中選擇，包括 Claude Sonnet 4 與 GPT-5。
