...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯與理解程式碼。它由與 GitHub Copilot coding agent 相同的 agentic harness 驅動，在深度整合 GitHub 工作流程的同時，提供智慧協助。
...
我們將 GitHub Copilot coding agent 的能力直接帶到你的終端機。透過 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI agent 協作。

- **原生終端機開發體驗：** 直接在命令列中與 Copilot coding agent 協作，不需要切換上下文。
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的儲存庫、issue 與 pull request，並沿用你現有的 GitHub 帳號完成驗證。
- **Agentic 能力：** 與 AI 協作者一起建置、編輯、除錯與重構程式碼，並可規劃及執行複雜任務。
- **由 MCP 驅動的可擴充性：** coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers 以擴充能力。
- **完整掌控：** 每個動作在執行前都可先預覽，未經你明確同意不會執行任何操作。

我們仍處於早期階段，但在你的回饋協助下，我們正快速迭代，致力讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
如果你目前尚未登入 GitHub，系統會提示你使用 `/login` 斜線指令。輸入此指令並依照畫面指示完成驗證。
