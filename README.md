...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯與理解程式碼。它採用與 GitHub Copilot coding agent 相同的 agentic harness，能在深度整合 GitHub 工作流程的同時，提供智慧化協助。
...
我們正把 GitHub Copilot coding agent 的強大能力直接帶到你的終端機中。透過 GitHub Copilot CLI，你可以在本機與一個理解你的程式碼與 GitHub 情境的 AI agent 同步協作。
...
- **MCP 驅動的擴充性：** coding agent 預設隨附 GitHub 的 MCP server，且支援自訂 MCP server 來延伸能力。
- **完整掌控：** 每個動作執行前都可先預覽，沒有任何操作會在未經你明確核准前發生。

我們仍在這段旅程的早期階段，但有了你的回饋，我們正快速迭代，目標是讓 GitHub Copilot CLI 成為你終端機中最好的夥伴。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，以提供更強的程式碼智慧能力。這項功能可提供像是前往定義、懸停資訊與診斷等智慧程式碼功能。

### 安裝 Language Servers

Copilot CLI 不會內建 LSP server。你需要另外安裝它們。以 TypeScript 支援為例，可這樣設定：
...
可在互動式工作階段中使用 `/lsp` 指令來檢查已設定的 LSP server，或直接檢視你的設定檔。
