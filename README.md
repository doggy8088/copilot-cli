...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它由與 GitHub Copilot coding agent 相同的代理式執行框架驅動，在與你的 GitHub 工作流程深度整合的同時，提供智慧協助。
...
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的儲存庫、議題與 pull request，全部透過你現有的 GitHub 帳號完成驗證。
- **代理式能力：** 與能夠規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **由 MCP 驅動的可擴充性：** 善用 coding agent 預設隨附 GitHub MCP server 這項優勢，也支援自訂 MCP server 來擴充能力。
- **完整掌控：** 執行前可預覽每一個動作，未經你明確批准不會發生任何操作。
...
預設情況下，`copilot` 使用 Claude Sonnet 4.5。執行 `/model` slash command 以從其他可用模型中選擇，包括 Claude Sonnet 4 與 GPT-5。
