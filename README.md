...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它採用與 GitHub Copilot coding agent 相同的 agentic harness，在提供智慧協助的同時，也與你的 GitHub 工作流程深度整合。
...
我們正將 GitHub Copilot coding agent 的強大能力直接帶進你的終端機。透過 GitHub Copilot CLI，你可以在本機與一個理解你的程式碼與 GitHub 情境的 AI agent 同步協作。

- **終端機原生開發：** 直接在命令列中與 Copilot coding agent 協作，不需要切換上下文。
- **內建 GitHub 整合：** 使用自然語言存取你的 repositories、issues 和 pull requests，全部都以你現有的 GitHub 帳號完成驗證。
- **Agentic 能力：** 與能規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **MCP 驅動的可擴充性：** 善用 coding agent 預設隨附的 GitHub MCP server，並支援自訂 MCP servers 來延伸能力。
