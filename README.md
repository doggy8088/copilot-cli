...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它由與 GitHub 的 Copilot coding agent 相同的代理式執行框架所驅動，在與你的 GitHub 工作流程深度整合的同時，提供智慧協助。
...
我們正將 GitHub Copilot coding agent 的能力直接帶到你的終端機。透過 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI 代理協作。
...
- **MCP 驅動的可擴充性：** 善用 coding agent 預設隨附 GitHub MCP 伺服器這一點，並支援自訂 MCP 伺服器以擴充能力。
...
如果你是透過組織或企業取得 GitHub Copilot 的使用權，而你的組織擁有者或企業管理員已在組織或企業設定中停用它，則你無法使用 GitHub Copilot CLI。更多資訊請參閱 [在你的組織中管理 GitHub Copilot 的政策與功能](http://docs.github.com/copilot/managing-copilot/managing-github-copilot-in-your-organization/managing-github-copilot-features-in-your-organization/managing-policies-for-copilot-in-your-organization)。
...
設定 `PREFIX` 以安裝到 `$PREFIX/bin/` 目錄。若以 root 執行，預設為 `/usr/local`；若以非 root 使用者執行，預設為 `$HOME/.local`。
...
我們開發得很快。預期會有頻繁更新，請讓你的客戶端保持在最新狀態，以取得最新功能與修正！
