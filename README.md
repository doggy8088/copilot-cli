...
GitHub Copilot CLI 直接將 AI 驅動的程式設計協助帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它由與 GitHub 的 Copilot coding agent 相同的代理式執行框架驅動，在與你的 GitHub 工作流程深度整合的同時，提供智慧協助。
...
- **由 MCP 驅動的可擴充性：** 受益於 coding agent 預設隨附 GitHub MCP server，並支援自訂 MCP servers 來延伸能力。
- **完整掌控：** 每個動作在執行前都能先預覽，沒有你的明確核准就不會發生任何事。

我們仍處於發展早期，但在你的回饋幫助下，我們正快速迭代，讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
- **Autopilot mode:** Autopilot 是一種新模式（按 `Shift+Tab` 可循環切換模式），會促使 agent 持續工作直到任務完成。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，可提供更進階的程式碼智慧功能。這項功能提供前往定義、懸停資訊與診斷等智慧程式碼能力。
...
我們建置的速度很快。預期會有頻繁更新，請讓你的用戶端保持最新，以取得最新功能與修正！
