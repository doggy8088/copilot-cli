...
GitHub Copilot CLI 將 AI 驅動的程式開發協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯與理解程式碼。它由與 GitHub Copilot coding agent 相同的代理式執行框架驅動，在深度整合 GitHub 工作流程的同時，提供智慧化協助。
...
我們將 GitHub Copilot coding agent 的能力直接帶進你的終端機。透過 GitHub Copilot CLI，你可以在本機與一個理解你的程式碼與 GitHub 情境的 AI agent 同步協作。

- **原生終端機開發：** 直接在命令列中使用 Copilot coding agent，不需要切換情境。
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的儲存庫、議題與 Pull Request，並直接沿用你現有的 GitHub 帳號完成驗證。
- **Agent 式能力：** 與能夠規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **以 MCP 驅動的可擴充性：** coding agent 預設內建 GitHub 的 MCP server，並支援自訂 MCP servers 以延伸能力。
- **完整掌控：** 執行前可預覽每一個動作，沒有任何操作會在未經你明確核准前發生。

我們仍處於早期階段，但在你的回饋協助下，我們正快速迭代，致力讓 GitHub Copilot CLI 成為你終端機中最好的夥伴。
...
如果你是透過所屬組織或企業使用 GitHub Copilot，而你的組織擁有者或企業管理員已在組織或企業設定中停用 GitHub Copilot CLI，則你無法使用 GitHub Copilot CLI。更多資訊請參閱 [Managing policies and features for GitHub Copilot in your organization](http://docs.github.com/copilot/managing-copilot/managing-github-copilot-in-your-organization/managing-github-copilot-features-in-your-organization/managing-policies-for-copilot-in-your-organization)。
...
第一次啟動時，你會看到我們可愛的動畫橫幅。如果你想再次顯示它，可在啟動 `copilot` 時加上 `--banner` 旗標。

如果你目前尚未登入 GitHub，系統會提示你使用 `/login` slash command。輸入此指令並依照畫面上的步驟完成驗證。
...
啟用後，此設定會持久化儲存到你的設定中，因此後續啟動時不再需要 `--experimental` 旗標。

#### Experimental Features

- **Autopilot mode：** Autopilot 是一種新模式（按 `Shift+Tab` 可循環切換模式），會鼓勵 agent 持續工作直到任務完成。

每次你向 GitHub Copilot CLI 提交 prompt，都會使你每月的 premium requests 配額減少一次。關於 premium requests，請參閱 [About premium requests](https://docs.github.com/copilot/managing-copilot/monitoring-usage-and-entitlements/about-premium-requests)。

如需更多 GitHub Copilot CLI 的使用資訊，請參閱[官方文件](https://docs.github.com/copilot/concepts/agents/about-copilot-cli)。

## 🔧 設定 LSP Servers

GitHub Copilot CLI 支援 Language Server Protocol (LSP)，可提供更強的程式碼智慧能力。此功能提供像是跳至定義、懸停資訊與診斷等智慧程式碼功能。
...
我們的開發速度很快。請預期會有頻繁更新，並務必讓你的用戶端保持最新，以取得最新功能與修正。

你的意見非常重要。歡迎在此儲存庫開 issue、加入 Discussions，並從 CLI 執行 `/feedback` 來提交保密回饋問卷。
