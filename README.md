...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它由與 GitHub 的 Copilot coding agent 相同的 agentic harness 驅動，在深度整合你的 GitHub 工作流程的同時，提供智慧化協助。
...
- **原生終端機開發：** 直接在命令列中與 Copilot coding agent 協作，無須切換情境。
...
- **MCP 驅動的可擴充性：** 利用 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers 來擴充能力。
