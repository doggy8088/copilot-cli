...
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的儲存庫、議題與拉取請求，並直接使用現有的 GitHub 帳號完成驗證。
- **代理式能力：** 與能規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **由 MCP 驅動的擴充性：** 善用 coding agent 預設搭載 GitHub MCP 伺服器的優勢，並支援自訂 MCP 伺服器來擴充能力。
- **完整控制：** 在執行前預覽每一個動作，未經你的明確批准，任何操作都不會發生。

我們仍處於早期階段，但在你的回饋幫助下，我們正快速迭代，致力讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
- **Autopilot mode:** Autopilot 是一種新模式（按 `Shift+Tab` 可在模式間切換），它會鼓勵代理持續工作，直到任務完成。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，以提供更強的程式碼智慧功能。這項功能可提供跳至定義、懸停資訊與診斷等智慧程式碼能力。
...
我們正在快速開發中。請預期會有頻繁更新，並讓你的用戶端保持最新，以取得最新功能與修正。
