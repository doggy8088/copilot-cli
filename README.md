...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它採用與 GitHub 的 Copilot coding agent 相同的代理式執行框架，在深度整合 GitHub 工作流程的同時，提供智慧輔助。
...
我們正將 GitHub Copilot coding agent 的強大能力直接帶進你的終端機。有了 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI agent 協作。
...
- **MCP 驅動的可擴充性：** 由於 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers，因此你可以進一步擴充能力。
- **完整控制：** 在執行前預覽每個動作，沒有你的明確批准，任何事情都不會發生。

我們仍處於旅程早期，但在你的回饋幫助下，我們正快速迭代，目標是讓 GitHub Copilot CLI 成為你終端機中最好的夥伴。
...
啟用後，這項設定會持久化到你的 config，因此後續啟動時不再需要 `--experimental` 旗標。

#### 實驗功能

- **Autopilot 模式：** Autopilot 是一種新模式（按 `Shift+Tab` 可在模式間切換），會鼓勵 agent 持續工作直到任務完成。
...
GitHub Copilot CLI 支援 Language Server Protocol (LSP)，以提供更進階的程式碼智慧功能。這項功能提供像是跳至定義、懸浮資訊與診斷等智慧程式碼能力。
