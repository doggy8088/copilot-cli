...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它由與 GitHub Copilot 編碼代理相同的代理式 harness 提供支援，在深度整合 GitHub 工作流程的同時，也能提供智慧協助。
...
我們正把 GitHub Copilot 編碼代理的能力直接帶進你的終端機。有了 GitHub Copilot CLI，你可以在本機以同步方式與理解你的程式碼與 GitHub 情境的 AI 代理協作。

- **終端機原生開發：** 直接在命令列中使用 Copilot 編碼代理，不需要切換上下文。
...
- **完整掌控：** 執行前可預覽每個動作，沒有你的明確批准就不會發生任何事。

我們的旅程仍在早期階段，但在你的回饋協助下，我們正快速迭代，致力讓 GitHub Copilot CLI 成為你終端機中最理想的夥伴。
...
第一次啟動時，你會看到我們可愛的動畫橫幅！如果你想再次看到這個橫幅，可以用 `--banner` 旗標啟動 `copilot`。

如果你目前尚未登入 GitHub，系統會提示你使用 `/login` slash 指令。輸入這個指令後，依照畫面指示完成驗證。
...
預設情況下，`copilot` 使用 Claude Sonnet 4.5。執行 `/model` slash 指令即可從其他可用模型中選擇，包括 Claude Sonnet 4 與 GPT-5。
...
- 在 CLI 中使用 `/experimental` slash 指令

啟用後，這項設定會持久化到你的設定檔，因此後續啟動時就不再需要 `--experimental` 旗標。
...
GitHub Copilot CLI 支援 Language Server Protocol（LSP），以提供更強的程式碼智慧功能。這項功能可提供如跳至定義、懸停資訊與診斷等智慧程式碼能力。

### 安裝 Language Servers

Copilot CLI 不會內建 LSP 伺服器。你需要另外安裝它們。例如，要設定 TypeScript 支援：
...
可在互動式工作階段中使用 `/lsp` 指令檢查已設定的 LSP 伺服器，或直接查看你的設定檔。
...
我們正在快速建置。預期會有頻繁更新，請讓你的客戶端保持最新，以取得最新功能與修正。

你的洞見非常寶貴！歡迎在這個儲存庫中開 issue、加入 Discussions，並從 CLI 執行 `/feedback` 提交一份保密的回饋問卷。
