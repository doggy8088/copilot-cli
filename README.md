...
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的 repositories、issues 與 pull requests，全部都以你現有的 GitHub 帳號完成驗證。
- **Agentic 能力：** 與能夠規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **MCP 驅動的可擴充性：** 由於 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers，因此你可以進一步擴充功能。
...
如果你目前尚未登入 GitHub，系統會提示你使用 `/login` slash command。輸入此指令，並依照畫面上的說明完成驗證。
...
每次你向 GitHub Copilot CLI 提交一則 prompt，你每月的 premium requests 配額就會減少一次。如需 premium requests 的資訊，請參閱 [About premium requests](https://docs.github.com/copilot/managing-copilot/monitoring-usage-and-entitlements/about-premium-requests)。
...
對於其他語言，請安裝對應的 LSP 伺服器，並依照下方展示的相同模式進行設定。
...
你可以在互動式工作階段中使用 `/lsp` 指令查看已設定的 LSP 伺服器，或直接查看你的設定檔。
