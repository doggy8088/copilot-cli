...
GitHub Copilot CLI 將 AI 驅動的程式設計協助直接帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它採用與 GitHub 的 Copilot coding agent 相同的代理式執行框架，在與你的 GitHub 工作流程深度整合的同時，提供智慧化協助。
...
我們正將 GitHub Copilot coding agent 的強大能力直接帶到你的終端機。透過 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你的程式碼與 GitHub 情境的 AI agent 協作。
...
- **MCP 驅動的可擴充性：** 善用 coding agent 預設內建 GitHub MCP server 的優勢，並支援自訂 MCP servers 以擴充能力。
- **完整掌控：** 執行前可預覽每一項動作，沒有你的明確核准，任何事情都不會發生。

我們的旅程仍在早期階段，但在你的回饋下，我們正快速迭代，讓 GitHub Copilot CLI 成為你終端機中最好的夥伴。
...
如果你是透過組織或企業取得 GitHub Copilot 的存取權，而你的組織擁有者或企業管理員已在組織或企業設定中停用 GitHub Copilot CLI，則無法使用 GitHub Copilot CLI。更多資訊請參閱 [Managing policies and features for GitHub Copilot in your organization](http://docs.github.com/copilot/managing-copilot/managing-github-copilot-in-your-organization/managing-github-copilot-features-in-your-organization/managing-policies-for-copilot-in-your-organization)。
...
設定 `PREFIX` 可安裝到 `$PREFIX/bin/` 目錄。預設為 `/usr/local`
...
3. 產生你的 token
4. 透過環境變數 `GH_TOKEN` 或 `GITHUB_TOKEN`（依此前後順序決定優先順序）將 token 加入你的環境
...
啟用後，這項設定會保存到你的設定中，因此後續啟動時不再需要 `--experimental` 旗標。
...
GitHub Copilot CLI 支援 Language Server Protocol（LSP），以提供更強的程式碼智慧能力。這項功能提供像是跳至定義、懸浮資訊與診斷等智慧型程式碼功能。
...
我們正在快速打造這個產品。請預期會有頻繁更新，請讓你的用戶端保持最新，以取得最新功能與修正！
