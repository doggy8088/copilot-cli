...
GitHub Copilot CLI 直接將 AI 驅動的程式設計輔助帶到你的命令列，讓你能透過自然語言對話來建置、除錯並理解程式碼。它採用與 GitHub 的 Copilot coding agent 相同的代理式 harness，在與 GitHub 工作流程深度整合的同時提供智慧協助。
...
我們正把 GitHub Copilot coding agent 的強大能力直接帶進你的終端機。透過 GitHub Copilot CLI，你可以在本機以同步方式與理解你的程式碼和 GitHub 情境的 AI agent 一起工作。

- **終端機原生開發：** 直接在命令列中使用 Copilot coding agent，無需切換情境。
- **開箱即用的 GitHub 整合：** 使用自然語言存取你的 repositories、issues 和 pull requests，全部都透過你現有的 GitHub 帳號完成驗證。
- **代理式能力：** 與能夠規劃並執行複雜任務的 AI 協作者一起建置、編輯、除錯與重構程式碼。
- **MCP 驅動的擴充能力：** 善用 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers 來擴充能力。
...
預設情況下，`copilot` 使用 Claude Sonnet 4.5。執行 `/model` slash command 可從其他可用模型中選擇，包括 Claude Sonnet 4 和 GPT-5。
...
- **Autopilot mode:** Autopilot 是一種新模式（按 `Shift+Tab` 可在模式間切換），會促使 agent 持續工作直到任務完成。
...
GitHub Copilot CLI 支援 Language Server Protocol（LSP），以提供更強的程式碼智慧功能。此功能可提供像是前往定義、懸停資訊和診斷等智慧型程式碼功能。
