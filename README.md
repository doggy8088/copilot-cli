...
我們將 GitHub Copilot coding agent 的強大能力直接帶進你的終端機。使用 GitHub Copilot CLI，你可以在本機以同步方式與一個理解你程式碼與 GitHub 情境的 AI agent 協作。
...
- **由 MCP 驅動的可擴充性：** 善用 coding agent 預設隨附 GitHub 的 MCP server，並支援自訂 MCP servers 以擴充能力。
- **完整掌控：** 每個動作在執行前都可先預覽，沒有任何事情會在未經你明確核准下發生。
...
你可以在互動式工作階段中使用 `/lsp` 指令查看已設定的 LSP servers，或直接查看你的設定檔。
